...
conn = pymysql.connect(host='127.0.0.1', port=3306, user='root',
                       password='root', db='echarts', charset='utf8', connect_timeout=1000)
# 创建数据库对象
cursor = conn.cursor()
...
cursor.execute(sql)
with open('诈骗数据.csv', 'r', encoding='utf-8') as csv1:
...
# # 高校总诈骗金额 (univMoney)
# sql_univMoney = 'create table if not exists univMoney (univName varchar(255),' \
#                 'money int, primary key (`univName`))'
# cursor.execute(sql_univMoney)
# sql_univMoney1 = 'truncate table univMoney'
# cursor.execute(sql_univMoney1)
# sql_univMoney2 = 'insert into univMoney select distinct schoolName univName,' \
#                  'sum(money) money from 诈骗数据 group by schoolName'
# cursor.execute(sql_univMoney2)
# 月份总诈骗金额 (monthMoney)
